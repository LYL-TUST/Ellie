...
Ellie 就是包在模型外面的控制循环：负责组 prompt、解析模型输出、
...
class Ellie:
    def __init__(
        ...
        self.run_store = run_store or RunStore(Path(workspace.repo_root) / ".ellie" / "runs")
    ...
    def ask_stream(self, user_message):
        """流式版本：逐 chunk yield 模型输出。"""
        refresh = self.refresh_prefix()
        self.resume_state = self.evaluate_resume_state()
        prompt, metadata = self._build_prompt_and_metadata(user_message)

        if not hasattr(self.model_client, "complete_stream"):
            # 降级：不支持流式则一次返回
            yield self.ask(user_message)
            return

        for chunk in self.model_client.complete_stream(prompt, self.max_new_tokens):
            yield chunk
    ...
    def parse(raw):
        ...
        if "<tool>" in raw and ("<final>" not in raw or raw.find("<tool>") < raw.find("<final>")):
            body = Ellie.extract(raw, "tool")
            try:
                payload = json.loads(body)
            except Exception:
                return "retry", Ellie.retry_notice("model returned malformed tool JSON")
            if not isinstance(payload, dict):
                return "retry", Ellie.retry_notice("tool payload must be a JSON object")
            if not str(payload.get("name", "")).strip():
                return "retry", Ellie.retry_notice("tool payload is missing a tool name")
            args = payload.get("args", {})
            if args is None:
                payload["args"] = {}
            elif not isinstance(args, dict):
                return "retry", Ellie.retry_notice()
            return "tool", payload
        if "<tool" in raw and ("<final>" not in raw or raw.find("<tool") < raw.find("<final>")):
            payload = Ellie.parse_xml_tool(raw)
            if payload is not None:
                return "tool", payload
            return "retry", Ellie.retry_notice()
        if "<final>" in raw:
            final = Ellie.extract(raw, "final").strip()
            if final:
                return "final", final
            return "retry", Ellie.retry_notice("model returned an empty <final> answer")
        raw = raw.strip()
        if raw:
            return "final", raw
        return "retry", Ellie.retry_notice("model returned an empty response")
    ...
    def parse_xml_tool(raw):
        ...
        if not match:
            return None
        attrs = Ellie.parse_attrs(match.group("attrs"))
        ...
        for key in ("content", "old_text", "new_text", "command", "task", "pattern", "path"):
            if f"<{key}>" in body:
                args[key] = Ellie.extract_raw(body, key)
