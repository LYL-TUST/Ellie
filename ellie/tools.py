...
BASE_TOOL_SPECS = {
    ...
    "git_diff": {
        ...
        "risky": False,
        "description": "Show git diff of current workspace changes.",
    },
    "retrieve": {
        "schema": {"query": "str", "top_k": "int=5"},
        "risky": False,
        "description": "Search the local document index for relevant information.",
    },
    "web_fetch": {
        "schema": {"url": "str", "timeout": "int=15"},
        "risky": False,
...
TOOL_EXAMPLES = {
    ...
    "git_diff": '<tool>{"name":"git_diff","args":{"staged":false}}</tool>',
    "delegate": '<tool>{"name":"delegate","args":{"task":"inspect README.md","max_steps":3}}</tool>',
    "retrieve": '<tool>{"name":"retrieve","args":{"query":"deployment process","top_k":3}}</tool>',
    "web_fetch": '<tool>{"name":"web_fetch","args":{"url":"https://docs.python.org/3/library/json.html","timeout":15}}</tool>',
}
...
def tool_retrieve(context, args):
    from .features.retrieval import search as retrieval_search

    query = str(args.get("query", "")).strip()
    top_k = int(args.get("top_k", 5))
    results = retrieval_search(query, context.root / ".ellie" / "index", top_k=top_k)
    if not results:
        return "(no relevant documents found — try running `ellie ingest <path>` first)"
    lines = []
    for r in results:
...
def tool_web_fetch(context, args):
    url = str(args["url"]).strip()
    timeout = int(args.get("timeout", 15))
    try:
        req = urllib.request.Request(url, headers={"User-Agent": "ellie/1.0"})
    ...
    if "text/html" in content_type:
        text = regex.sub(r"<script[^>]*>.*?</script>", "", raw, flags=regex.S | regex.I)
        text = regex.sub(r"<style[^>]*>.*?</style>", "", text, flags=regex.S | regex.I)
