...
IGNORED_PATH_NAMES = {".git", ".ellie", "__pycache__", ".pytest_cache", ".ruff_cache", ".venv", "venv"}
    ...
    def build(cls, cwd, repo_root_override=None):
        ...
        def git(args, fallback=""):
            try:
                ...
                if not raw:
                    return fallback
                # Git 在 Windows 上通常以 UTF-8 输出路径，但 subprocess
                # 在中文系统上可能用 GBK/CP936 解码。我们直接读字节，
                # 先用 UTF-8 解码——如果失败再回退到 Python 默认编码。
                try:
                    return raw.decode("utf-8").strip() or fallback
                except UnicodeDecodeError:
                    return raw.decode(sys.getdefaultencoding()).strip() or fallback
            except Exception:
                return fallback
        ...
        for base in (repo_root, cwd):
            for name in DOC_NAMES:
                ...
                try:
                    ...
                    # 文档不在 repo_root 下（如 --cwd 指向的目录不是 git 仓库，
                    # 或编码问题导致路径不匹配），直接用绝对路径做 key。
