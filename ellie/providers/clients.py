...
OPENAI_COMPATIBLE_USER_AGENT = "ellie/0.1"
...
class FakeModelClient:
    ...
    def complete_stream(self, prompt, max_new_tokens):
        result = self.complete(prompt, max_new_tokens)
        yield result
...
class OpenAICompatibleModelClient:
    ...
    def complete(self, prompt, max_new_tokens, prompt_cache_key=None, prompt_cache_retention=None):
        ...
        在 agent 链路里的位置：
        它位于 `Ellie.ask()` 的模型调用阶段，是稳定前缀缓存复用链路真正
        落到 provider API 的地方。
        """
        self.last_completion_metadata = {}
        payload = {
    ...
    def complete_stream(self, prompt, max_new_tokens, prompt_cache_key=None, prompt_cache_retention=None):
        self.last_completion_metadata = {}
        payload = {
            "model": self.model,
...
class VLLMModelClient:
    """Model client for vLLM or any OpenAI Chat Completions-compatible backend.

    vLLM 暴露的是标准 OpenAI Chat Completions API（/v1/chat/completions），
    跟 Ellie 现有的 OpenAICompatibleModelClient（走 /responses 的 Responses API）不同。
    这个 client 专门适配 Chat Completions 格式的 payload 和响应解析。

    用法：
        在 .env 中设置：
            ELLIE_VLLM_API_BASE=https://your-tunnel.seetacloud.com:8443
            ELLIE_VLLM_API_KEY=not-needed
            ELLIE_VLLM_MODEL=Qwen2.5-7B-AWQ
        然后：
            uv run ellie --provider vllm
    """

    def __init__(self, model, base_url, api_key, temperature, timeout, verify_ssl=True):
        self.model = model
        self.base_url = _normalize_versioned_base_url(base_url)
        self.api_key = api_key
        self.temperature = temperature
    ...
    def complete(self, prompt, max_new_tokens, **kwargs):
        """向 OpenAI Chat Completions 接口发起一次模型调用。

        vLLM 原生格式的请求体比 Responses API 简单：
        - messages 直接是 {"role": "user", "content": prompt}
        - max_tokens 控制输出长度
        - stream=False 走非流式，避免 SSE 解析的复杂性
        """
        self.last_completion_metadata = {}
        payload = {
            "model": self.model,
        ...
        # seetacloud / AutoDL 隧道使用自签名证书，SSL 验证会失败。
        # 用户通过 verify_ssl=False 可以跳过证书校验。
        context = None
        ...
        for attempt in range(attempts):
            try:
                ...
                if attempt < attempts - 1:
                    ...
                    continue
                raise RuntimeError(
                    "Could not reach the vLLM backend.\n"
                    f"Base URL: {self.base_url}\n"
                    f"Model: {self.model}"
                ) from exc
        ...
        try:
            ...
            raise RuntimeError(
                "vLLM error: backend returned non-JSON content that could not be parsed"
            ) from exc
        if data.get("error"):
            raise RuntimeError(f"vLLM error: {data['error']}")

        # 复用 _extract_openai_text，它已经支持 choices[0].message.content 格式。
        text = _extract_openai_text(data)
        self.last_completion_metadata = _extract_usage_cache_details(data)
        if text:
            return text
        raise RuntimeError("vLLM error: could not extract text from response")
