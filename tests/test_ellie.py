...
def build_agent(tmp_path, outputs, **kwargs):
    ...
    store = SessionStore(tmp_path / ".ellie" / "sessions")
...
def test_list_files_hides_internal_agent_state(tmp_path):
    agent = build_agent(tmp_path, [])
    (tmp_path / ".ellie").mkdir(exist_ok=True)
    (tmp_path / ".git").mkdir(exist_ok=True)
    (tmp_path / "hello.txt").write_text("hi\n", encoding="utf-8")

    result = agent.run_tool("list_files", {})

    assert ".ellie" not in result
...
def test_welcome_screen_keeps_box_shape_for_long_paths(tmp_path):
    deep = tmp_path / "very" / "long" / "path" / "for" / "the" / "Ellie" / "agent" / "welcome" / "screen"
    ...
    assert "MINI CODING AGENT" not in welcome
    assert "Ellie" in welcome
    assert "your terminal companion" in welcome
...
def test_build_arg_parser_leaves_provider_unset_for_runtime_resolution(tmp_path):
    args = ellie_pkg.build_arg_parser().parse_args(["--cwd", str(tmp_path)])

    assert args.provider is None


def test_build_arg_parser_accepts_anthropic_provider(tmp_path):
    args = ellie_pkg.build_arg_parser().parse_args(["--cwd", str(tmp_path), "--provider", "anthropic"])

    assert args.provider == "anthropic"


def test_build_arg_parser_accepts_deepseek_provider(tmp_path):
    args = ellie_pkg.build_arg_parser().parse_args(["--cwd", str(tmp_path), "--provider", "deepseek"])
...
def test_build_agent_uses_project_env_provider_when_cli_omitted(tmp_path):
    ...
    with patch.dict(os.environ, {}, clear=True):
        with patch(
            ...
            "ellie.cli.AnthropicCompatibleModelClient",
...
def test_build_agent_prefers_cli_provider_over_project_env_provider(tmp_path):
    ...
    args = ellie_pkg.build_arg_parser().parse_args(
    ...
    with patch.dict(os.environ, {}, clear=True):
        with patch(
            ...
            "ellie.cli.OpenAICompatibleModelClient",
            side_effect=AssertionError("openai client should not be used"),
        ), patch("ellie.cli.AnthropicCompatibleModelClient") as mock_anthropic:
...
def test_build_agent_uses_anthropic_default_model_when_env_is_missing(tmp_path):
    ...
    with patch.dict(
        ...
        with patch("ellie.cli.AnthropicCompatibleModelClient") as mock_anthropic:
            ellie_pkg.build_agent(args)
...
def test_explicit_memory_promotion_persists_durable_memory_topics(tmp_path):
    agent = build_agent(
        ...
        [
            ...
            "Project convention: Preserve local agent state under .ellie/.\n"
    ...
    index_path = tmp_path / ".ellie" / "memory" / "MEMORY.md"
    ...
    assert report["durable_promotions"] == [
        ...
        "project-conventions: Preserve local agent state under .ellie/.",
...
def test_explicit_memory_promotion_rejects_secret_shaped_and_transient_lines(tmp_path):
    ...
    report = json.loads(agent.run_store.report_path(agent.current_task_state).read_text(encoding="utf-8"))
    conventions_path = tmp_path / ".ellie" / "memory" / "topics" / "project-conventions.md"
    dependency_path = tmp_path / ".ellie" / "memory" / "topics" / "dependency-facts.md"
...
def test_package_import_surface_includes_cli_entrypoints():
    assert callable(ellie_pkg.main)
    assert callable(ellie_pkg.build_agent)
    assert callable(ellie_pkg.build_arg_parser)


def test_module_execution_help_works():
    result = subprocess.run(
        [sys.executable, "-m", "ellie", "--help"],
        capture_output=True,
        text=True,
    )
